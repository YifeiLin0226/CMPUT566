...
model_urls = {
    'resnet50': 'resnet50-25c4b509.pth',
    'resnet101': 'resnet101-2a57e44d.pth',
    'resnet152': 'resnet152-0d43d698.pth'
}
...
def resnet(n_layers):
    ...
    if n_layers == 50:
        ...
        model.load_state_dict(torch.load(pretrained_dir/ model_urls['resnet50']))
    elif n_layers == 101:
        model = ResNet(Bottleneck, [3, 4, 23, 3])
        model.load_state_dict(torch.load(pretrained_dir / model_urls['resnet101']))
    elif n_layers == 152:
        model = ResNet(Bottleneck, [3, 8, 36, 3])
        model.load_state_dict(torch.load(pretrained_dir / model_urls['resnet152']))
    else:
        raise ValueError("=> unknown n_layers: {}".format(n_layers))
    return model
